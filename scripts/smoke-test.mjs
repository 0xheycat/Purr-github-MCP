...
try {
  ...
  const createRepo = tools.find((tool) => tool.name === 'create_repository');
  if (getFile?.annotations?.readOnlyHint !== true || deleteFile?.annotations?.destructiveHint !== true || verifyMcp?.annotations?.readOnlyHint !== true || createRepo?.annotations?.destructiveHint !== false) {
    throw new Error(`Tool annotations are missing or incorrect: ${JSON.stringify({ getFile, deleteFile, verifyMcp, createRepo })}`);
  }
