...
<script>
const root=document.querySelector("#app");let card=normalize(window.openai?.toolOutput,window.openai?.toolResponseMetadata);let expanded=false;render();
window.addEventListener("openai:set_globals",event=>{const g=event.detail?.globals||{};const next=normalize(g.toolOutput??window.openai?.toolOutput,g.toolResponseMetadata??window.openai?.toolResponseMetadata);update(next)}, {passive:true});
window.addEventListener("message",event=>{const message=event.data;if(message?.jsonrpc!=="2.0"||message.method!=="ui/notifications/tool-result")return;update(normalize(message.params))},{passive:true});
